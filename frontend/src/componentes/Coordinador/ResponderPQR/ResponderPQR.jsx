...
// ---------------------------------------------------------------
// 🎯 Componente: Tarjeta Individual de PQR
// ---------------------------------------------------------------
const PqrCard = ({ pqr, isSelected, onSelect }) => {
  ...
  return (
    <tr className={isSelected ? "table-active" : ""}>
      ...
      <td>
        <div className="user-info">
          <div className="user-name text-body-secondary">
            {pqr.acudiente?.persona?.nombre}{" "}
            {pqr.acudiente?.persona?.apellido}
          </div>
          <small className="text-muted">
            {pqr.acudiente?.persona?.correo}
          </small>
        </div>
      </td>
      ...
      <td>
        <span
          className={`badge ${getEstadoBadge(pqr.estado?.nombre)}`}
        >
          {pqr.estado?.nombre}
        </span>
      </td>
    ...
    <div className="historial-header">
      <div className="user-info">
        <span className="user-avatar">
          {item.usuario_respuesta?.username?.charAt(0).toUpperCase() ||
            "S"}
        </span>
      ...
      <span
        className={`estado-badge estado-${item.estado_historial?.nombre?.toLowerCase()}`}
      >
        {item.estado_historial?.nombre}
      </span>
    </div>

    <div className="historial-mensaje">{item.mensaje}</div>
  </div>
);
...
const ResponderPqr = () => {
  ...
  // ------------------ Seleccionar PQR ------------------
  const manejarSeleccionPqr = useCallback(
    async (idPqr) => {
          ...
          {selectedPqr ? (
            ...
            <div className="empty-selection">
              <div className="empty-content">
                ...
                <p>
                  Elige una solicitud de la lista para ver el historial y
                  responder
                </p>
              </div>
            </div>
