  ...
  try {
    ...
    const historiales = await db.Observacion.findAll({
      include: [
        ...
        {
          model: db.GravedadObservacion, // 👈 agregamos gravedad
          as: 'gravedad',
          attributes: ['id_gravedad', 'nombre'],
          required: false
        }
      ],
      order: [
    ...
    // 🔄 Mapear para enviar la gravedad de forma consistente
    const resultado = historiales.map(obs => ({
      ...obs.toJSON(),
      gravedad: obs.gravedad ? { nombre: obs.gravedad.nombre } : null
    }));
